    ...
    try:
        ...
        if len(commands) < 2:
            log(
                command,
                no_mistake=False,
                name_error="ERROR: Не указан файл или каталог для удаления",
            )
        ...
        if commands[1] == "-r":
            ...
            if len(commands) < 3:
                log(
                    command,
                    no_mistake=False,
                    name_error="ERROR: Не указан каталог для рекурсивного удаления",
                )
                return (
                    f"{date_time()} ERROR: Не указан каталог для рекурсивного удаления"
                )
            ...
            if limit_path is None:
                if os.path.exists(norm_path) and os.path.isdir(norm_path):
                    ...
                    log(
                        command,
                        no_mistake=False,
                        name_error="ERROR: Рекурсивное удаление доступно только для каталогов",
                    )
                    ...
                    log(
                        command,
                        no_mistake=False,
                        name_error=f"ERROR: Нет каталога с названием {path}",
                    )
            ...
            if os.path.exists(norm_path) and os.path.isfile(norm_path):
                ...
                log(
                    command,
                    no_mistake=False,
                    name_error=f"ERROR: {path} является каталогом. Для удаления каталога используйте -r",
                )
                ...
                log(
                    command,
                    no_mistake=False,
                    name_error=f"ERROR: В каталоге нет файла {path}",
                )
...
def limited_paths(command, path):
    ...
    if abs_path == os.path.abspath(".") or path == ".":
        log(
            command,
            no_mistake=False,
            name_error="ERROR: Нельзя удалять текущий каталог",
        )
    ...
    # Родительский каталог
        ...
        log(
            command,
            no_mistake=False,
            name_error="ERROR: Нельзя удалять родительский каталог",
        )
    ...
    # Корневой каталог
        ...
        log(
            command,
            no_mistake=False,
            name_error="ERROR: Нельзя удалять домашнюю директорию",
        )
    ...
    # Корневой каталог (проверка для всех ОС)
        ...
        or path in ["/", "\\"]  # проверка корневых каталогов (кроме Windows)
        or (os.name == "nt" and len(abs_path) == 3 and abs_path.endswith(":\\"))
    ):  # проверка для Windows
        log(
            command,
            no_mistake=False,
            name_error="ERROR: Нельзя удалять корневой каталог",
        )
...
def remove_directory(command, name_file):
    ...
    print("Подтверждение")
    print(
        f'Элемент в "{name_file}" имеет дочерние объекты.'
