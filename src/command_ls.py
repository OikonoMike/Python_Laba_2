...
def function_ls(command):
    ...
    try:
        ...
        log(
            command,
            no_mistake=False,
            name_error=f"ERROR: Неправильный формат ввода команды: {str(error)}",
        )
        return f"{date_time()} ERROR: Неправильный формат ввода команды: {str(error)}"

    original_directory = (
        os.getcwd()
    )  # сохраняем оригинальный катало, чтобы потом в него возвращаться
    try:
        # запрос вида ls
        ...
        # запрос вида ls [path] или ls -l
            ...
            # если вида ls [path]
                ...
                result_cd = function_cd(
                    f"cd {command_list[1]}"
                )  # переходим по пути, который указал пользователь
                ...
                log(
                    command,
                    no_mistake=False,
                    name_error="ERROR: Неправильный формат ввода",
                )
            ...
            if command_list[2] == "-l":
                ...
                if result_cd is None:
                    ...
                    result_l = function_minus_l(
                        command
                    )  # сохраняем файлы из каталога, в который мы перешли
...
def function_minus_l(command):
    ...
    for file in os.listdir():
        ...
        result.append(
            f"{mode}     {write_time: %d.%m.%Y %H:%M:%S}              {size_file}    {file}"
        )
    log(command, no_mistake=True, name_error="")
    return "\n".join(result)
