    ...
    try:
        ...
        if command_list[1] == '-r':
            if len(command_list) < 4:
                log(command, no_mistake=False, name_error='ERROR: Не указан источник и/или каталог для рекурсивного копирования')
            ...
            if os.path.isdir(normpath_for_source):
                normpath_for_destination = os.path.normpath(os.path.expanduser(command_list[3]))

                if not os.path.exists(normpath_for_source):
                    log(command, no_mistake=False, name_error=f'ERROR: ERROR: Каталога {command_list[2]} не найдено')
                ...
                try:
                    ...
                    log(command, no_mistake=False, name_error=f'ERROR: Нет прав доступа для копирования')
                ...
                log(command, no_mistake=False, name_error=f'ERROR: Опция -r поддерживается только для копирования каталогов')
        ...
        # Обработка копирования файлов (без опции -r)
            ...
            normpath_for_destination = os.path.normpath(os.path.expanduser(command_list[2]))

            if not os.path.exists(normpath_for_source):
                log(command, no_mistake=False, name_error=f'ERROR: Файла {command_list[1]} не найдено')
                return f'{date_time()} ERROR: Файла {command_list[1]} не найдено'

            if os.path.isdir(normpath_for_source):
                log(command, no_mistake=False, name_error='ERROR: Для копирования каталога нужно использовать опцию -r')
                return f'{date_time()} ERROR: Для копирования каталога нужно использовать опцию -r'

            if not os.path.isfile(normpath_for_source):
                log(command, no_mistake=False, name_error=f'ERROR: {command_list[1]} не является файлом')
                return f'{date_time()} ERROR: {command_list[1]} не является файлом'

            # Определяем путь назначения
            if os.path.isdir(normpath_for_destination): # Если назначение - каталог, то сохраняем исходное имя файла
                final_path_to_destination = os.path.join(normpath_for_destination, os.path.basename(normpath_for_source))
            ...
            if os.path.abspath(normpath_for_source) == os.path.abspath(final_path_to_destination):
                log(command, no_mistake=False, name_error='ERROR: Невозможно скопировать файл в самого себя')
                return f'{date_time()} ERROR: Невозможно скопировать файл в самого себя'

            # Копирование файла
            try:
                shutil.copy2(normpath_for_source, final_path_to_destination) # Копирование файла с сохранением метаданных
                log(command, no_mistake=True, name_error='')
                return None
            except PermissionError:
                log(command, no_mistake=False, name_error=f'ERROR: Нет прав доступа для копирования')
